...
2. [Retrieve the availability in cache](#availability-in-cache)
3. [Credit Report](#get-credit-report)
...
## Availability in cache

You can send the details of your customer to check if their financial data is cached or not. The API returns the confidence score along with the availability based on the profile configured by the partner.
